const RANKS = {
    ...
    desc: {
        rank: {
            ...
            '4': "reduce mass upgrade 3 cost scaling by 20%.",
            '5': "mass upgrade 2 boosts itself.",
            '6': "boost mass gain by (Rank+1)^2.",
            '13': "triple mass gain.",
            '14': "double Rage Powers gain.",
            '17': "make rank 6 reward better. [(Rank+1)^2 -> (Rank+1)^(Rank^1/3)]",
            '34': "make mass upgrade 3 softcap start 1.2x later.",
            '40': "increase tickspeed power based on ranks.",
            '45': "ranks boost Rage Powers gain.",
            '90': "rank 40 reward becomes stronger.",
            ...
            '800': "make mass gain softcap 0.25% weaker per rank.",
        },
        tier: {
            '1': "reduce rank reqirements by 20%.",
            '2': "raise mass gain by 1.15",
            '3': "reduce all mass upgrades cost scaling by 20%.",
            '4': "add +5% tickspeed power for every tier, softcaps after +40%.",
            '6': "boost Rage Powers by Tier.",
            '8': "make tier 6's reward stronger based on dark matters.",
            '12': "double tier 4's effect, and remove its softcap.",
            '30': "stronger effect's softcap is 10% weaker.",
            '55': "improve rank 380's effect based on tier.",
            '100': "Super Tetr starts 5 later.",
        },
        tetr: {
            ...
            '4': "Super Rank scales weaker based on Tier, Super Tier scales 20% weaker.",
		...
		oct: {
			...
			'10': "Oct boosts Infinity/Eternal Mass.",
			'12': "Oct boosts Hept 52's effect.",
			...
			'34': "Oct boosts Exotic Matter gain.",
    ...
    effect: {
        ...
        pent: {
            '2'() {
                let ret = E(1.3).pow(player.ranks.tetr.softcap("1e2000000",0.6,2).softcap("e2e30",0.2,2));
                return ret
            },
            '4'() {
...
function updateRanksHTML() {
    ...
    if (tmp.rank_tab == 0) {
		...
		if(player.ranks.enne.gte(100)){
			...
			for(let i=9;i<=getHighestBeyondRank()+1;i++){
...
function beyondRankTier(x) {
	...
	if(beyondRankTier(x-1).lt(100))return E(0);
	return beyondRankTier(x-1).sub(100).max(0).root(1.5).div(5).add(1).floor();
}
